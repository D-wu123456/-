...
def table_is_null(locate,log_,dict):
    for key, value in dict.items():
        if '' in key or ' ' in key:
            if locate == '鉴定人':
                tools.write_log(log_, locate, '鉴定人信息', '未填写或为空')
            else:
                tools.write_log(log_, locate, '检材编号', '未填写或为空')
            continue
        for key_,value_ in value.items():
            if '' in key_ or ' ' in key_:
                tools.write_log(log_, locate, '检材属性', '未填写或为空')
            if '' in value_ or ' ' in value_:
                tools.write_log(log_, locate, '属性值', '未填写或为空')


def get_tables(log_,case,filename):
    # 获取鉴定意见内所有的的表格
    ...
    for i in range(len(table_text_list)):
        if len(table_text_list[i]) == 0:
            # 表格未填写这里注意可以优化
            tools.write_log(log_, '检材表格和鉴定人', '表格内容', '未放置')
        else:
    ...
    if len(index_list) ==1:
        if index_list[0] == 0:
            tools.write_log(log_, '检材照片', '表格内容', '未放置')
        else:
            tools.write_log(log_, '检材', '表格内容', '未放置')
    if len(index_list) == 2:
        if index_list[0] +2 != index_list[1]:
            tools.write_log(log_, '鉴定人', '表格内容', '未放置')
        else:
            tools.write_log(log_, '检材和检材照片表格', '表格内容', '未放置')
    if len(index_list) == 3:
        if index_list[0] == 0:
            tools.write_log(log_, '检材照片表格', '表格内容', '未放置')
        else:
            tools.write_log(log_, '检材表格', '表格内容', '未放置')

    print(index_list)
    table_list_jiancai = []  # 检材列表
    table_list_jiancaizhaop = []  # 检材照片列表
    table_list_jiandingren = []  # 鉴定人以及签名列表
    # 获取到三张表只存有数据，过滤掉多余数据，根据出现次数判断，这里仍然需要优化
    for i in range(index_list[1], index_list[2]):
        # if '照片' not in table_text_list[i][1]:#过滤检材照片
        sum = 0
        for j in range(index_list[1], index_list[2]):
            if table_text_list[i][0] == table_text_list[j][0]:
                sum += 1
        if sum >= 4 and '.' not in table_text_list[i]:
            table_list_jiancaizhaop.append(table_text_list[i])
    jiandingren_dict = {}
    temp_dict = {}
    for i in range(index_list[2], len(table_text_list)):
        temp_dict.setdefault(table_text_list[i][0], table_text_list[i][1])
        if i - index_list[2] == 1:
            jiandingren_dict.setdefault('第一鉴定人', temp_dict)
            temp_dict = {}
        if i - index_list[2] == 3:
            jiandingren_dict.setdefault('第二鉴定人', temp_dict)
    ...
    # 查看是否有未填写内容
    table_is_null('鉴定材料',log_,dict_jiancai_dict)
    table_is_null('检材照片',log_,dict_jiancaizhaop_dict)
    table_is_null('鉴定人',log_,jiandingren_dict)
    #
    #
    #查看填写内容是否标准
