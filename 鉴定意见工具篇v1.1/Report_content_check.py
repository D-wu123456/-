...
def main():
    #修改案件编号和鉴定意见的位置
    case='2022-251'
    filename='./2022-251鉴定意见书.docx'
    #获取除了表格之外所有的数据
    text_dict=getText(filename)

    #获取鉴定意见内检材、检材照片以及鉴定人信息的表格
    #字典类型、字典类型、列表类型
    tools.get_tables(filename)

    dict_jiancai,dict_jiancaizhaop,table_list_jiandingren=tools.get_tables(filename)
    # print(dict_jiancai)
    # print(dict_jiancaizhaop)
    # print(table_list_jiandingren)

    now_date,time_name=tools.get_log_name()
    log_='./报告检测日志/'+case + time_name

    with open(log_+'.txt','a+',encoding='utf-8') as wirte:
        wirte.write(now_date+'\n'+'本次案件编号是:'+case+'\n')#后期优化加上检测报告的时间
    print(text_dict)

    tools.tools.case_detec(log_,case,text_dict.get('鉴定过程'),text_dict.get('分析说明'),text_dict.get('鉴定意见'),text_dict.get('保存和签名'),dict_jiancai,dict_jiancaizhaop)
